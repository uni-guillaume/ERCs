...
3. The config file MUST contain two JSON keys:
   ...
   - `enforcement` containing one of `lax, strict, none`
...
- In `lax` mode: Wallets MUST NOT display any warning or negative UX for unsigned requests. This mode allows dapps to implement TWIST incrementally, signing critical flows (like swaps) while other flows remain unsigned without causing user alarm. Wallets should only display positive indicators (e.g., a padlock icon) when signed requests are successfully verified.
