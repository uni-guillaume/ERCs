...
- In `strict` mode: When dealing with unsigned requests, either because the dapp (or an attacker) uses `request` on a domain where TWIST is configured, or because the signature does not match, wallets should warn the user but
not block. An eloquently worded warning will increase the transparency enough that end users may opt to cancel the interaction or proceed with the unsigned call.
