...
3. The config file MUST contain two JSON keys:
   - `publicKeys` containing an array of objects of the form `{ id, alg, publicKey }`
   - `enforcement` containing one of `lax, strict, none` (determines whether unsigned requests trigger warnings in `strict` mode, or proceed silently in `lax` mode)
...
1. Upon receiving an [EIP-1193](./eip-1193.md) call, the wallet MUST check for the existence of the TWIST manifest for the `sender.tab.url` domain
   a. The wallet MUST enforce HTTPS as HTTP calls would be vulnerable to DNS spoofing
   b. The wallet MUST verify that the manifest is hosted on the `sender.tab.url` domain
   c. The wallet SHOULD find the DNS TXT record to locate the manifest
...
3. If TWIST is configured, the wallet MUST read the `enforcement` field from the manifest and proceed based on the method being called:
   a. If `enforcement` is set to `none`, then proceed as usual without any TWIST verification, regardless of the method
   b. If the `wallet_signedRequest` method is used with parameters `requestPayload`, `signature` and `keyId`:
      ii. The wallet MUST verify that the `keyId` exists in the TWIST manifest and find the associated key record
      iii. From the key record, the wallet MUST use the `alg` field and the `publicKey` field to verify `requestPayload` integrity by calling `crypto.verify(alg, key, signature, requestPayload)`
      iv. If the signature is valid, the wallet SHOULD display a visible cue indicating that this interaction is signed
        - The wallet MUST then call `request` with the argument `requestPayload`
      v. If the signature is invalid, the wallet MUST display a visible and actionable warning to the user:
        - If the user opts to ignore the warning, then proceed to call `request` with the argument `requestPayload`
        - If the user opts to cancel, then the wallet MUST cancel the call
   c. If the `request` method is used (unsigned request):
      i. If `enforcement` is `strict`, the wallet MUST display a visible and actionable warning to the user:
         - If the user opts to ignore the warning, then proceed as usual
         - If the user opts to cancel, then the wallet MUST cancel the call
      ii. If `enforcement` is `lax`, the wallet MUST NOT display any warning or negative UX, and proceed as usual
...
async function signedRequest(
  ...
  // 3. Parse the manifest and get the key, algo, and enforcement mode
  ...
  // 5. Display positive indicator for valid signatures
  displaySignedIndicator();
...
Similarly to the padlock icon for HTTPS, wallets should display a visible indication when TWIST is configured on a domain and a signed request is being made. This will improve the UX of the end user who will immediately be able to tell
that interactions between the dapp they are using and the wallet are secure, and this will encourage dapp developers to adopt TWIST, making the overall ecosystem more secure.

The enforcement mode specified in the manifest determines how wallets should handle unsigned requests:

- In `strict` mode: When dealing with unsigned requests (when the dapp uses `request` instead of `wallet_signedRequest`), wallets should display a visible and actionable warning to the user but not block. An eloquently worded warning will increase the transparency enough that end users may opt to cancel the interaction or proceed with the unsigned call.

- In `lax` mode: Wallets MUST NOT display any warning or negative UX for unsigned requests. This mode allows dapps to implement TWIST incrementally, highlighting where TWIST is enabled, without any negative effect where it is not. Wallets should only display positive indicators (e.g., a padlock icon) when signed requests are successfully verified.

- In `none` mode: TWIST verification is disabled entirely. This is intended for development and testing purposes.

Note: Invalid signatures (when `wallet_signedRequest` is used but the signature verification fails) MUST trigger warnings regardless of the enforcement mode, as specified in the signature verification section.
...
The inclusion of three enforcement modes (`strict`, `lax`, and `none`) provides flexibility for dapps to adopt TWIST incrementally. This is particularly important for complex dapps with multiple transaction flows, as it allows them to implement TWIST for critical operations first, while maintaining a positive user experience for flows that have not yet been migrated. The `lax` mode ensures users are not presented with warnings for legitimate unsigned transactions, which could lead to warning fatigue and reduced security awareness. The `strict` mode provides maximum security for dapps that have fully implemented TWIST across all flows, while the `none` mode facilitates development and testing.
